...
MyGame.render.Random = (function (graphics) {
    ...
    function generateYValue(mean, stdDev, canvasHeight) {
        ...
        while (y > canvasHeight * 0.9 || y < canvasHeight * 0.4) {
    ...
    function generateMidPoints(x1, y1, x2, y2, canvasHeight) {
        ...
        let distance = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - 1));
        let midX = (x1 + x2) / 2;
        let midY = 0.5 * (y1 + y2) + (2 * nextGaussian(0, 1) * Math.abs(x2 - x1));
        while (midY > Math.max(y2, y1) + distance || midY < Math.min(y2, y1) - distance || midY > canvasHeight * 0.9 || midY < canvasHeight * 0.4)
